...
func ExtractTable[T any](rows []T, tagName string) [][]string {
	...
	for i := 0; i < len(rows); i++ {
		row := rows[i]
		var r reflect.Value
		if reflect.ValueOf(row).Kind() == reflect.Ptr {
...
func getColumnNames(r reflect.Value, tagName string) []string {
	...
	for i := 0; i < rt.NumField(); i++ {
		...
		if field.Anonymous && r.Field(i).Kind() == reflect.Struct {
			values = append(values, getColumnNames(r.Field(i), tagName)...)
		} else {
...
func getCellValues(r reflect.Value, tagName string) []string {
	...
	for j := 0; j < rt.NumField(); j++ {
		...
		if field.Anonymous && r.Field(j).Kind() == reflect.Struct {
			values = append(values, getCellValues(r.Field(j), tagName)...)
		} else {
